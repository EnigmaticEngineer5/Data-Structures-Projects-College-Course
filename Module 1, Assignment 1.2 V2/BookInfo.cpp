...
BookInfo& BookInfo::operator=(const BookInfo& sourceBookInfo) {
	if (this != &sourceBookInfo) {
		this->title = sourceBookInfo.getTitle();
		this->author = sourceBookInfo.getAuthor();
		this->publisher = sourceBookInfo.getPublisher();
	}

	return *this;
}

const bool BookInfo::operator==(const BookInfo& sourceBookInfo) const {
