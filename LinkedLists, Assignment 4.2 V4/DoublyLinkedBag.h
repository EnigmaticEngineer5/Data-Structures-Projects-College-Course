...
#include "../LinkedLists, Assignment 4.2 V2/BagInterface.h"
...
inline Node<ItemType>* DoublyLinkedBag<ItemType>::rotateNodes(const ItemType& targetItem) const
{
	...
	while (cin.fail() || direction < 1 || direction > 2) {
		if (cin.fail()) {
			...
			cout << "\nInvalid input, please re-enter a valid integer --> ";
		}
		else {
			cout << "\nInvalid selection, choose an option between one and two --> ";
		}
		cin >> direction;
	}
...
inline  Node<ItemType>* DoublyLinkedBag<ItemType>::leftRotate(const ItemType& target, Node<ItemType>* curPtr) const
{
	...
	while (curPtr && curPtr->getItem() != target) {
		curPtr = curPtr->getPrev();
	}
