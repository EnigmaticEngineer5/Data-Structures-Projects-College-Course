...
#include "../Binary Trees, Assignment 9.1/BinaryNodeTree.h"
...
protected:
	...
	BinaryNode<ItemType>* findNode(BinaryNode<ItemType>* treePtr, const ItemType&
		target, bool& success) const;
...
template < class ItemType>
BinaryNode<ItemType>* BinarySearchTree<ItemType>::insertInorder(BinaryNode<ItemType>*
...
template < class ItemType>
BinaryNode<ItemType>* BinarySearchTree<ItemType>::removeNode(BinaryNode<ItemType>
	* nodePtr)
{
	...
	if (nodePtr->isLeaf())
	{
		...
		if (nodePtr->isChild())
		{
			...
			delete nodePtr;
			nodePtr = nullptr;
			cout << "Tiene un hijo\n" << nodeToConnectPtr->getItem() << endl;
			return nodeToConnectPtr;
		}
		else // N has two children
		{
			// Find the inorder successor of the entry in N: it is in the left subtree 
			//rooted
				// at N’s right child
...
BinarySearchTree<ItemType>::removeLeftmostNode(BinaryNode<ItemType>* nodePtr,
	ItemType& inorderSuccesssor)
{
	if (nodePtr->getLeftChildPtr() == nullptr)
	{
		// This is the node you want; it has no left child, but it might have a 
	/*	right subtree*/
...
template < class ItemType>
BinaryNode<ItemType>* BinarySearchTree<ItemType>::findNode(BinaryNode<ItemType>*
	subTreePtr, const ItemType& target, bool& success) const
{
	if (subTreePtr == nullptr)
	{
		success = false;
...
template<class ItemType>
inline void BinarySearchTree<ItemType>::destroyTree(BinaryNode<ItemType>* subTreePtr)
{
	if (subTreePtr != nullptr)
	{
...
template<class ItemType>
inline BinaryNode<ItemType>* BinarySearchTree<ItemType>::copyTree(const BinaryNode<ItemType>* treePtr) const
{
...
{
	if (isEmpty()) {
		cout << "TreeException:EmptyTree" << endl;
		return ItemType();
	}
...
ItemType BinarySearchTree<ItemType>::getEntry(const ItemType& anEntry) const
{
	...
	if (!isSuccessful)
	{
		cout << "TreeException: Entry not found in tree!" << endl;
	}
